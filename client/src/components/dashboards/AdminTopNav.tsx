...
import { Bell, Settings, User, LogOut, LayoutDashboard, Shield, Activity } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { clearStoredUser, getStoredUser } from '../../utils/auth';
import { getUnreadNotificationCount, mockNotifications, mockActivities } from '../../utils/mockData';
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
              <div className="px-4 py-4 border-b border-gray-100">
                ...
                <span className="inline-flex mt-2 px-3 py-1 rounded-full bg-[#008800] text-[#FFFDD0] text-xs font-semibold">
                  Admin
                </span>
              </div>
              ...
              <div className="py-2">
                ...
                <Link to="/admin/dashboard" className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 text-gray-700">
                  <LayoutDashboard size={18} /> Dashboard
                </Link>
                <Link to="/admin/system" className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 text-gray-700">
              ...
              <div className="border-t border-gray-100 px-4 py-3">
                <div className="flex items-center gap-2 mb-3">
                  <Activity size={16} className="text-[#9A7B4F]" />
                ...
                <div className="space-y-2">
                  {recentActivities.map((a) => (
                    <div key={a.id} className="text-sm text-gray-600">
                      <span className="font-semibold">{a.userName}</span> {a.action} {a.target}
                    </div>
                  ))}
                </div>
              </div>
